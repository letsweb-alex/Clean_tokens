...
5. Escalate to IMBA if needed.  
   Apply a colder, more deterministic framing when the source text is heavily contaminated or high-stakes.

### IMBA

IMBA stands for Isolation Memory Barrier Alignment.

IMBA mode is the hard-reset layer. Use it when the source prompt is bloated, infected by persona leakage, or impossible to execute as written.
...
```text
Run CLEAN TOKENS.
Treat the incoming material as contaminated with semantic residue.
